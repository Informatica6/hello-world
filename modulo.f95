...
! Potencia(A,k,Ak)  matriz A, k= pontencia a la que elevas, ak= matriz hecha la potencia-------46                                                                        
! Inversa (A,AI) matriz A a la que quieres hacer la inversa Ai matriz inversa------------------70
! Gauss(A,b,x) Ax=b----------------------------------------------------------------------------148
! FactorizacionLU(A,Al,Au) A matriz que quieres factorizar en LU Al(lower), Au(upper)----------204
! GaussFactorLU(A,b,X) Ax=b--------------------------------------------------------------------265
! MiniCu(A) A devulve los coeficientes del polinomio-------------------------------------------314
! Jacobi(A,b,Tol,x) Ax=b , donde tol es la tolerancia------------------------------------------361
! Norma(Vector,n) hace el modulo de un vector de tamaño n--------------------------------------429
! GAUSS_SEIDEL(A,b,x,ITE) Ax=b, donde ITE es la iteraciones------------------------------------450
!----------------------------------------------------------------------------------------------------
